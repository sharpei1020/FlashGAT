...
import tvm.sparse
from sparsetir_spmm.utils import get_dataset, ell
...
def get_opt_spmm(
    ...
    global cached_bucketing_format
    if cached_bucketing_format is None:
        indptr_nd = tvm.nd.array(indptr.type(th.IntTensor).numpy(), device=tvm.cpu())
        indices_nd = tvm.nd.array(indices.type(th.IntTensor).numpy(), device=tvm.cpu())
        cached_bucketing_format = column_part_hyb(
            m, n, indptr_nd, indices_nd, num_col_parts, bucket_sizes
        )
    row_indices, col_indices, mask = cached_bucketing_format
