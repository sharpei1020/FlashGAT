...
struct atom {
  ...
  float         str;    // Total weighted degree of the community members
  vint          child;  // Last vertex that is merged to this vertex
...
vint merge(const vint v, std::vector<edge>* const nbrs, graph* const g) {
  ...
  const vint ch = g->vs[v].a->child;
  const atom v_a = g->vs[v].a.exchange(atom(-1, ch));  
  const float vstr = v_a.str;
