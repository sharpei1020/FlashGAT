...
std::vector<torch::Tensor> rabbit_reorder(
  ...
  std::vector<edge_list::edge> edges(numedges);
  ...
  torch::Tensor new_ord = torch::empty({static_cast<int64_t>(numnodes)}, {in_edge_index.options()});
  auto new_ord_ptr = new_ord.accessor<int, 1>();

  torch::Tensor out_edge_index = torch::empty_like(in_edge_index);
  ...
  for(int i = 0; i < numnodes; i++){
    new_ord_ptr[mapping[i]] = i;
  }

  return {out_edge_index, new_ord};
}
