...
__global__ void agnn_kernel_16x8(
    ...
    for (; i<(block_num-1); i++) {
        ...
        // Softmax
        {
            ...
            uint32_t col_mask = *(uint16_t*)&ColMask[colmask_offset+i*2];
...
__global__ void agnn_kernel_16x16(
    ...
    for (; i<(block_num-1); i++) {
        ...
        // Softmax
        {
            ...
            int row_id = (lane_id>>2)+warp_id*8;
            uint32_t row_mask = *(uint16_t*)&RowMask[row_start_byte+__popc(col_mask&((1<<row_id)-1))*2];
            for (int k = 0; k < 4; k++) {
...
__global__ void agnn_kernel_8x8_3(
    ...
    asm volatile("mma.sp::ordered_metadata.sync.aligned.m16n8k16.row.col.f32.tf32.tf32.f32 {%0, %1, %2, %3}, {%4, %5, %6, %7}, "
        ...
        "f"(alpha[0]), "f"(alpha[1]), "f"(alpha[2]), "f"(alpha[3]), "r"(E[0]));
    for (int k = 0; k < 2; k++)
    //     atomicAdd(&sparse_A[i&1][(threadIdx.x>>2)&7][(threadIdx.x&3)*2+k], alpha[k]+alpha[k+2]);
        sparse_A[i&1][(threadIdx.x>>3)+8*k][threadIdx.x&7] = alpha[k]+alpha[k+2];
    sparse_A[(i+1)&1][threadIdx.x>>3][threadIdx.x&7] = 0.f;
    ...
    for (; i<(block_num-2); i++) {
        ...
        asm volatile("mma.sp::ordered_metadata.sync.aligned.m16n8k16.row.col.f32.tf32.tf32.f32 {%0, %1, %2, %3}, {%4, %5, %6, %7}, "
            ...
            "f"(alpha[0]), "f"(alpha[1]), "f"(alpha[2]), "f"(alpha[3]), "r"(E[0]));
        for (int k = 0; k < 2; k++)
        //     atomicAdd(&sparse_A[(i+1)&1][(threadIdx.x>>2)&7][(threadIdx.x&3)*2+k], alpha[k]+alpha[k+2]);
            sparse_A[(i+1)&1][(threadIdx.x>>3)+k*8][threadIdx.x&7] = alpha[k]+alpha[k+2];
        {
            ...
            for (int k = 0; k < 2; k++) {
                mask[k] = (col_mask>>((threadIdx.x>>2)&7))&(row_mask>>((threadIdx.x&3)+k*4))&1;
...
at::Tensor AGNN_adaptive(
    ...
    switch(select) {
        ...
        case 8:
            ...
            break;
        case 3:
            agnn_kernel_8x8_3<<<block_8x8, 64, 0, stream[0]>>>(
                ...
                output.data_ptr<float>(),
                num_nodes);
            agnn_kernel_8x16<<<block_8x16, 32, 0, stream[1]>>>(
        ...
        case 5:
            agnn_kernel_8x8_3<<<block_8x8, 64, 0, stream[0]>>>(
                ...
                output.data_ptr<float>(),
                num_nodes);
            agnn_kernel_16x8<<<block_16x8, 64, 0, stream[1]>>>(
        ...
        case 9:
            agnn_kernel_8x8_3<<<block_8x8, 64, 0, stream[0]>>>(
                ...
                output.data_ptr<float>(),
                num_nodes);
            agnn_kernel_16x16<<<block_16x16, 64, 0, stream[1]>>>(
            ...
            break;
        case 6:
            agnn_kernel_8x16<<<block_8x16, 32, 0, stream[0]>>>(
                RowWindowId_8x16.data_ptr<int>(),
                RowWindowRowOffset.data_ptr<int>(),
                RowWindowBlockOffset.data_ptr<int>(),
                RowWindowSparseAToXOffset.data_ptr<int>(),
        ...
        case 10:
            ...
            break;
        case 12:
            agnn_kernel_16x8<<<block_16x8, 64, 0, stream[0]>>>(
                RowWindowId_16x8.data_ptr<int>(),
                RowWindowRowOffset.data_ptr<int>(),
                RowWindowBlockOffset.data_ptr<int>(),
                RowWindowSparseAToXOffset.data_ptr<int>(),
        ...
        case 7:
            ...
            agnn_kernel_8x16<<<block_8x16, 32, 0, stream[1]>>>(
                ...
                output.data_ptr<float>(),
                num_nodes);
            agnn_kernel_16x8<<<block_16x8, 64, 0, stream[2]>>>(
                RowWindowId_16x8.data_ptr<int>(),
                RowWindowRowOffset.data_ptr<int>(),
                RowWindowBlockOffset.data_ptr<int>(),
                RowWindowSparseAToXOffset.data_ptr<int>(),
        ...
        case 11:
            ...
            agnn_kernel_8x16<<<block_8x16, 32, 0, stream[1]>>>(
                ...
                output.data_ptr<float>(),
                num_nodes);
            agnn_kernel_16x16<<<block_16x16, 64, 0, stream[2]>>>(
                RowWindowId_16x16.data_ptr<int>(),
                RowWindowRowOffset.data_ptr<int>(),
                RowWindowBlockOffset.data_ptr<int>(),
                RowWindowSparseAToXOffset.data_ptr<int>(),
        ...
        case 13:
            ...
            break;
        case 14:
            agnn_kernel_8x16<<<block_8x16, 64, 0, stream[0]>>>(
                RowWindowId_8x16.data_ptr<int>(),
                RowWindowRowOffset.data_ptr<int>(),
                RowWindowBlockOffset.data_ptr<int>(),
                RowWindowSparseAToXOffset.data_ptr<int>(),
        ...
        case 15:
            ...
            agnn_kernel_16x8<<<block_16x8, 64, 0, stream[2]>>>(
                ...
                output.data_ptr<float>(),
                num_nodes);
            agnn_kernel_16x16<<<block_16x16, 64, 0, stream[3]>>>(
                RowWindowId_16x16.data_ptr<int>(),
                RowWindowRowOffset.data_ptr<int>(),
                RowWindowBlockOffset.data_ptr<int>(),
                RowWindowSparseAToXOffset.data_ptr<int>(),
