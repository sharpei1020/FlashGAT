...
__global__ void agnn_kernel_16x8_32(
    ...
    int cur_addr = __cvta_generic_to_shared(&dense_X[i&1][(threadIdx.x>>3)][lane_end*2]);
    asm volatile("cp.async.ca.shared.global [%0], [%1], 8;\n"::"r"(cur_addr), "l"(&x[SparseAToX_idx[i&1][threadIdx.x>>3]*32+(lane_id&15)*2]));
    ...
    for (i=block_start; i<(block_end-1); i+=1) {
        ...
        cur_addr = __cvta_generic_to_shared(&dense_X[(i+1)&1][(threadIdx.x>>3)][lane_end*2]);
        asm volatile("cp.async.ca.shared.global [%0], [%1], 8;\n"::"r"(cur_addr), "l"(&x[SparseAToX_idx[(i+1)&1][threadIdx.x>>3]*32+(lane_id&15)*2]));
        asm volatile("cp.async.commit_group;\n"::);
        // SDDMM
        float alpha[4] = {0.f};
        for (int j = 0; j < 4; j++) {
